...
import { CardFull, CardBig, CardHorizontal } from '@/components/ArticleCard';
...
export default async function HomePage() {
    ...
    const livingPosts   = await getCategoryPosts(categories, 'living', 4);
          ...
          <section className="section">
            <div className="container">
             ...
             <div className="articles-featured">
                <CardBig post={drinkingPosts[0]} />
              <div className="articles-stack">
{drinkingPosts.slice(1).map(post => <CardHorizontal key={post.id} post={post} />)}
                            </div>
                            </div>
                            </div>
                            </section>
...
{diningPosts.length > 0 && (
          <section className="section section-white">
            <div className="container">
             ...
             <div className="articles-featured">
                <CardBig post={diningPosts[0]} />
              <div className="articles-stack">
{diningPosts.slice(1).map(post => <CardHorizontal key={post.id} post={post} />)}
                          </div>
                          </div>
                          </div>
                          </section>
...
{livingPosts.length > 0 && (
          <section className="section">
            <div className="container">
             ...
             <div className="articles-featured">
                <CardBig post={livingPosts[0]} />
              <div className="articles-stack">
{livingPosts.slice(1).map(post => <CardHorizontal key={post.id} post={post} />)}
  </div>
                          </div>
                          </div>
                          </section>
